...
In the webmail, we find only two mails, one of which has the subject "DB Access". Upon reviewing the mail, we find the password for the root user of the database: `Fg-'kKXBj87E:aJ$`.
...
It appears to be an archive file, which means it contains multiple files. Next, we'll examine the content of the file.\
...
In this case, we can perform reverse engineering to retrieve its [source code](scripts/bomb_source.c).
...
This phase is also quite simple: we need to enter a series of 6 numbers. The first number must be 1, and each subsequent number is the product of the previous number and the current index + 1.
...
Let's dive straight into the files available in this session! There's a `README` file that contains the following:
...
To proceed, we need to obtain the memory addresses of `system` and the string `/bin/sh`. This will allow us to overwrite the return address, executing the `system` function with `/bin/sh`, thereby granting us a shell prompt.
...
We have obtained the `system` function address: `0xb7e6b060`. Now, let's proceed to find the address of the `/bin/sh` string in the libc, using **gdb** as well.
